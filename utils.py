...
class DatasetProcessor(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        file=file_id.replace("_matte", "")
        image_path = os.path.join(image_folder+ '/'+file + ".png")
       # print(image_path)
        
       #  mask_folder = '../data/matting/'
       # # print(mask_folder)
       #  file_id=file_id.replace("clip", "matting")
       #  mask_path = os.path.join(mask_folder+ '/'+file_id + ".png")
        mask_folder = self.root_path
        mask_path = os.path.join(mask_folder+ '/'+file_id + ".png")
       # print(mask_path)
       # os._exit() 
        ...
        if self.as_torch_tensor:
            ...
            if not self.is_test:
                ...
                mask = cv2.imread(str(mask_path),flags=cv2.IMREAD_UNCHANGED)
                if mask==[]:
                    print(mask_path)
                    print('wrong')
                #print(mask.shape)
               ...
               # mask=mask[:,:,3]
                ...
                #resize to 320x256
                image = cv2.resize(image, (512, 640), interpolation=cv2.INTER_LANCZOS4)
                mask = cv2.resize(mask, (512, 640), interpolation=cv2.INTER_LANCZOS4)
...
class Trainer:
    ...
    def __init__(self, path=None, gpu=-1, **kwargs):
        ...
        if path is not None:
            ...
            print(self.directory,self.model_name)
    ...
    def fit(self, dataset, dataset_val, kwargs):
     ...
     #   print(self.model.parameters)
        # pre_dic = torch.load('./data/mobilenetV2_model_checkpoint_metric.pth')
        # self.model.load_state_dict(pre_dic["state_dict"])
        # print('pre')
        ...
        for e in range(epoch):
            ...
            for image, mask, mask_w in tqdm(data.DataLoader(dataset, batch_size = batch_size, shuffle = True, num_workers=0)):
               ...
               # print(y_pred.shape)
                if self.model_type == "mobilenetv2":
                    ...
                    y_pred = nn.functional.interpolate(y_pred, scale_factor=2, mode='nearest')
            ...
            val_loss, val_metric = [], []
            with torch.no_grad():
                for image, mask, mask_w in data.DataLoader(dataset_val, batch_size = batch_size, shuffle = False, num_workers=0):
